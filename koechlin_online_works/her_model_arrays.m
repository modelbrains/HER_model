...
for i=1:length(stim_seq);
    ...
    %if it is a new block, purge memory
          if this_stim(1,1)<9
             curr_mem{1}=curr_mem{1}.*0;
              curr_mem{2}=curr_mem{2}.*0;
              curr_mem{3}=curr_mem{3}.*0;
        end
...
%% new block
%because this version of the model was not optimized for the task (model
%parameters from unified set in supplementary table, we've automated the
%mapping of external stimuli to internal representations here.  This is
%done for speed and consistency of learning - results are comparable when
%the optimized version of the model learns the external mapping
%automatically
  tmpcurr_mem=0.*curr_mem{1};
  tmpcurr_mem(this_stim(2,1))=1;
  curr_mem{1}=tmpcurr_mem;
  tmpcurr_mem=0.*curr_mem{2};
  tmpcurr_mem(this_stim(1,1))=1;
  curr_mem{2}=tmpcurr_mem;
  if this_stim(1,1)<9
     tmpcurr_mem=0.*curr_mem{3};
     tmpcurr_mem(this_stim(1,1))=1;
  curr_mem{3}=tmpcurr_mem;
  end
...
%%%%just take are, generate error signals, and pass those error signals up
%%%%the hierarchy.
